...
void ControlCenter::Load(physics::WorldPtr _parent, sdf::ElementPtr _sdf)
{
  ...
  this->addEntityConnection = event::Events::ConnectAddEntity(
      boost::bind(&ControlCenter::AddEntityToWorld, this, _1));
  this->updateConnection = event::Events::ConnectWorldUpdateBegin(
      boost::bind(&ControlCenter::OnSystemRunning, this, _1));
  // Perform extra initializations
  ExtraInitializationInLoad(_parent,_sdf);
} // ControlCenter::Load
void ControlCenter::ExtraInitializationInLoad(
...
} // ControlCenter::ExtraInitializationInLoad
LibraryTemplate *ControlCenter::DynamicallyLoadedLibrary(
   const char* library_path, void *lib_handle)
{
  ...
  return mkr();
} // ControlCenter::DynamicallyLoadedLibrary
void ControlCenter::CloseLoadedLibrary(void *lib_handle)
{
  dlclose(lib_handle);
} // ControlCenter::CloseLoadedLibrary
void ControlCenter::EnableAutoMagneticConnection(void)
{
  this->autoMagneticConnectionFlag = true;
}
void ControlCenter::AddEntityToWorld(std::string & _info)
{
  //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  transport::SubscriberPtr new_module_sub 
      = node->Subscribe(topic_name,&ControlCenter::FeedBackMessageDecoding, this);
  //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  if (autoMagneticConnectionFlag) {
    ...
    transport::SubscriberPtr new_world_sub = node->Subscribe(
        new_sub_name,&ControlCenter::AutomaticMagneticConnectionManagement, this);
    WorldColSubscriber.push_back(new_world_sub);
  }
} // ControlCenter::AddEntityToWorld
...
} // ControlCenter::OnSystemRunning
void ControlCenter::OnSystemRunningExtra(
    const common::UpdateInfo & _info){}
void ControlCenter::BuildConfigurationFromXML(string file_name)
{
...
} // ControlCenter::BuildConfigurationFromXML
void ControlCenter::BuildConnectionFromXML(string file_name)
{
  file<> xmlFile(file_name.c_str());
  xml_document<> doc;    // character type defaults to char
  doc.parse<0>(xmlFile.data());
...
} // ControlCenter::BuildConnectionFromXML
...
} // ControlCenter::FeedBackMessageDecoding
void ControlCenter::ExtraWorkWhenModelInserted(CommandMessagePtr &msg)
{
...
} // ControlCenter::ExtraWorkWhenModelInserted
...
} // ControlCenter::AutomaticMagneticConnectionManagement
// TODO: Since there is a huge change in this function, it should be tested
void ControlCenter::ConnectAndDynamicJointGeneration(
...
} // ControlCenter::ConnectAndDynamicJointGeneration
void ControlCenter::RotationQuaternionCalculation(math::Vector3 normal_axis,
...
} // ControlCenter::RotationQuaternionCalculation
void ControlCenter::NewPositionCalculation(SmoresEdgePtr an_edge,
...
} // ControlCenter::NewPositionCalculation
double ControlCenter::ConversionForAngleOverPi(double angle)
{
  if (abs(angle) > PI+0.0000000001){
    return angle>0?angle-2*PI:2*PI+angle;
  }
  return angle;
} // ControlCenter::ConversionForAngleOverPi
void ControlCenter::DynamicJointDestroy(SmoresEdgePtr edge)
{
...
} // ControlCenter::DynamicJointDestroy
...
void ControlCenter::PassiveConnect(SmoresModulePtr module_1, 
...
} // ControlCenter::PassiveConnect
void ControlCenter::PassiveConnect(SmoresModulePtr module_1, 
    SmoresModulePtr module_2, int node1_ID, int node2_ID)
{
  PassiveConnect(module_1, module_2, node1_ID, node2_ID, 0, 0);
} // ControlCenter::PassiveConnect
void ControlCenter::ActiveConnect(SmoresModulePtr module_1, 
    SmoresModulePtr module_2, int node1_ID, int node2_ID, 
    double node_angle, double node_distance)
{
  if (!AlreadyConnected(module_1, module_2, node1_ID, node2_ID))
...
} // ControlCenter::ActiveConnect
void ControlCenter::ActiveConnect(SmoresModulePtr module_1, 
    SmoresModulePtr module_2, int node1_ID, int node2_ID)
{
  ActiveConnect(module_1, module_2, node1_ID, node2_ID, 0, 0);
} // ControlCenter::ActiveConnect
void ControlCenter::Disconnect(SmoresEdgePtr edge)
{
...
void ControlCenter::Disconnect(SmoresModulePtr module, int node_ID)
{
  SmoresEdgePtr edge = module->GetNode(node_ID)->GetEdge();
  this->Disconnect(edge);
} // ControlCenter::Disconnect
void ControlCenter::Disconnect(string module_name, int node_ID)
{
  SmoresEdgePtr edge = 
      GetModulePtrByName(module_name)->GetNode(node_ID)->GetEdge();
  this->Disconnect(edge);
} // ControlCenter::Disconnect
void ControlCenter::Disconnect(string module_name1, string module_name2)
{
  for (unsigned int i = 0; i < connectionEdges.size(); ++i) {
...
} // ControlCenter::CommandManager
void ControlCenter::CommandExecution(ModuleCommandsPtr current_command_container)
{
...
} // ControlCenter::CommandExecution
void ControlCenter::SendGaitTable(SmoresModulePtr module, const bool *flag, 
...
void ControlCenter::SendGaitTable(SmoresModulePtr module, int joint_ID, 
...
void ControlCenter::SendGaitTable(SmoresModulePtr module, 
...
//   // command_message::msgs::CommandMessage connection_message;
//   connection_message->set_messagetype(2);
//   connection_message->set_priority(priority);
...
//   connection_message->mutable_positionneedtobe()->mutable_position()->set_z(orientation_angle);
...
//   // {
//   //   ModuleCommandsPtr new_command_message(new ModuleCommands(module));
//   //   new_command_message->CommandSquence.push_back(connection_message);
//   //   moduleCommandContainer.push_back(new_command_message);
//   //   module->moduleCommandContainer = new_command_message;
//   // }else{
//   //   if (priority == 0)
//   //   {
//   //     module->moduleCommandContainer->CommandSquence.push_back(connection_message);
//   //   }else
//   //   {
...
//   //       {
//   //         module->moduleCommandContainer->CommandSquence.insert(module->moduleCommandContainer->CommandSquence.begin()+i,connection_message);
//   //         break;
//   //       }
//   //     }
//   //   }
...
//   aNewMessage.ActualCommandMessage = connection_message;
...
//         CommandPtr connection_messageTmp(new command_message::msgs::CommandMessage());
//         connection_messageTmp->set_messagetype(3);
//         connection_messageTmp->set_priority(0);
//         for (int i = 0; i < 4; ++i)
//         {
//           connection_messageTmp->add_jointgaittablestatus(0);
//           connection_messageTmp->add_jointgaittable(0);
//         }
//         CommandPro TmpNewMessage;
//         TmpNewMessage.ActualCommandMessage = connection_messageTmp;
...
// }
void ControlCenter::SendGaitTableInstance(SmoresModulePtr module, 
    const bool *flag, const double *gait_value, int msg_type)
{
  ...
  module->ModulePublisher->Publish(*connection_message);
} // ControlCenter::SendGaitTableInstance
void ControlCenter::SendGaitTableInstance(SmoresModulePtr module, 
    const bool *flag, const double *gait_value)
{
  SendGaitTableInstance(module, flag, gait_value, 4);
} // ControlCenter::SendGaitTableInstance
void ControlCenter::SendPositionInstance(SmoresModulePtr module, 
  double x, double y, double orientation_angle)
{
  ...
  connection_message->mutable_positionneedtobe()->mutable_orientation()->set_w(0);
  module->ModulePublisher->Publish(*connection_message);
} // ControlCenter::SendPositionInstance
// TODO: Need to be tested
void ControlCenter::EraseComaands(SmoresModulePtr module) 
{
  if (module->moduleCommandContainer) {
    EraseCommandPtrByModule(module);
    module->moduleCommandContainer.reset();
  }
} // ControlCenter::EraseComaands
...
SmoresModulePtr ControlCenter::GetModulePtrByName(string module_name)
{
  ...
  return exist_module;
} // ControlCenter::GetModulePtrByName
...
} // ControlCenter::EraseCommandPtrByModule
int ControlCenter::GetModuleIndexByName(string module_name)
{
  int exist_module = -1;
  for (unsigned int i = 0; i < moduleList.size(); ++i) {
    if (module_name.compare(moduleList.at(i)->ModuleID)==0) {
  ...
  return exist_module;
} // ControlCenter::GetModuleIndexByName
bool ControlCenter::AlreadyConnected(SmoresModulePtr module_1, 
    SmoresModulePtr module_2, int node1_ID, int node2_ID)
{
...
} // ControlCenter::AlreadyConnected
bool ControlCenter::AlreadyConnected(SmoresModulePtr module_1, 
...
void ControlCenter::InterpretCommonGaitString(string a_command_str)
{
  Color::Modifier yellow_log(Color::FG_YELLOW);
  Color::Modifier def_log(Color::FG_DEFAULT);
  cout<<yellow_log<<"World: Command str: "<<a_command_str<<";"<<endl;
}
void ControlCenter::InterpretSpecialString(string a_command_str)
{
  Color::Modifier green_log(Color::FG_GREEN);
  Color::Modifier def_log(Color::FG_DEFAULT);
  cout<<green_log<<"World: Special command str: "<<a_command_str<<";"<<endl;
}
void ControlCenter::AddCondition(string condition_id)
{
  bool not_exist = true;
...
} // ControlCenter::AddCondition
void ControlCenter::FinishOneConditionCommand(string condition_id)
{
  for (unsigned int i = 0; i < commandConditions.size(); ++i) {
    if (commandConditions.at(i)->condition_id.compare(condition_id) == 0) {
...
} // ControlCenter::FinishOneConditionCommand
bool ControlCenter::CheckCondition(string condition_id)
{
  for (unsigned int i = 0; i < commandConditions.size(); ++i) {
    if (commandConditions.at(i)->condition_id.compare(condition_id) == 0) {
